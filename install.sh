...
EOF
			...
			if [ ! -f $setup_path/checkD.pl ];then
                    ...
                    cp 	$setup_path/checkD.pl /root/bt;
...
if [ "$kernelStatus" = "" ]; then
	if [ "$huaweiLogin" != "" ] && [ "$huaweiSys" != "" ]; then
		...
		cp kernel-headers-3.10.0-514.el7.x86_64.rpm /root/bt
		#rm -f kernel-headers-3.10.0-514.el7.x86_64.rpm
	else
		yum install kernel-headers -y
	fi
fi
cp kernel-headers.pl /root/bt
...
Install_setuptools()
{
	if [ ! -f "/usr/bin/easy_install" ];then
		...
		mkdir -p /root/bt
		cp setuptools-33.1.1.zip /root/bt
...
Install_psutil()
{
	...
	if [ "$isSetup" = "" ];then
		...
		cp psutil-5.2.2.tar.gz /root/bt
...
Install_mysqldb()
{
	...
	if [ "$isSetup" = "" ];then
		...
		cp MySQL-python-1.2.5.zip /root/bt
...
Install_chardet()
{
	...
	if [ "$isSetup" = "" ];then
		...
		cp chardet-2.3.0.tar.gz /root/bt
